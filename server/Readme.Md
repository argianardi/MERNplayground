...
- <details open>
  <summary><a href="#glosarium">Glosarium</a></summary>
   <ul>
    <li><a href="#format-applicationx-www-form-urlencoded">Format application/x-www-form-urlencoded</a></li>
...
- <details open>
    <summary><a href="#buat-error-handling">Buat Error Handling</a></summary>
     <ul>
      <li><a href="#middleware-error-not-found"> Middleware Error Not Found</a></li>
      <li><a href="#middleware-error-not-found">Middleware Error Handler</a></li>
      <li><a href="#menggunakan-middleware-error-di-file-entry-point">Menggunakan Middleware Error di File Entry Point</a></li>
      <li><a href="#buat-async-handler-menghindari-penggunaan-try-catch-berulang">Buat Async Handler Menghindari Penggunaan Try-Catch Berulang</a></li>
     </ul>
    </details>
- <details open>
    <summary><a href="#fitur-register-menggunakan-jwt">Fitur Register Menggunakan JWT</a></summary>
     <ul>
      <li><a href="#register-menggunakan-cookies"> Register Menggunakan Cookies</a></li>
      <li><a href="#register-tanpa-menggunakan-cookies">Register Tanpa Cookies</a></li>
     </ul>
    </details>
- <details open>
    <summary><a href="#fitur-login-menggunakan-jwt">Fitur Login Menggunakan JWT</a></summary>
     <ul>
      <li><a href="#login-menggunakan-cookies">Login Menggunakan Cookies</a></li>
      <li><a href="#login-tanpa-menggunakan-cookies">Login Tanpa Cookies</a></li>
     </ul>
    </details>
- [Proteksi endpoint, Logout, dan Get User](#proteksi-endpoint-logout-dan-get-user)
- [Membuat Fitur Create Product](#membuat-fitur-create-product)
- [Fitur Get All Products dan Get Product By Id](#fitur-get-all-products-dan-get-product-by-id)
- [Update Product](#update-product)
- [Delete Product](#delete-product)
- [Buat Authorization Berdasarkan Role](#buat-authorization-berdasarkan-role)
- [Upload file (image) menggunakan Multer](#upload-file-image-menggunakan-multer)
- [Cara Membuat Seed untuk Product](#cara-membuat-seed-untuk-product)
- [Menambahkan Filter dan Pagination pada Produk](#menambahkan-filter-dan-pagination-pada-produk)
- [Searching Berdasarkan Nama Product](#searching-berdasarkan-nama-product)
- [Membuat Fitur Post Order](#membuat-fitur-post-order)
- [Get All Orders, Get Order By Id dan Get Current User Order](#get-all-orders-get-order-by-id-dan-get-current-user-order)
- [Gunakan Middleware Express Mongo Sanitize Dan Helmet](#gunakan-middleware-express-mongo-sanitize-dan-helmet)
- [CORS di Backend](#cors-di-backend)
- <details open>
  <summary><a href="#gunakan-middleware-express-mongo-sanitize-dan-helmet">Gunakan Middleware Express Mongo Sanitize Dan Helmet</a></summary>
   <ul>
    <li><a href="#express-mongo-sanitize">Express Mongo Sanitize</a></li>
    <li><a href="#streamifier">Streamifier</a></li>
    <li><a href="#helmet">Helmet</a></li>
    <li><a href="#penggunaan-express-mongo-sanitize-dan-helmet">Penggunaan Express Mongo Sanitize dan Helmet</a></li>
   </ul>
  </details>
- <details open>
  <summary><a href="#upload-file-dengan-cloudinary-dan-streamifier-di-expressjs">Upload File dengan Cloudinary dan Streamifier di ExpressJS</a></summary>
   <ul>
    <li><a href="#cloudinary">Cloudinary</a></li>
    <li><a href="#streamifier">Streamifier</a></li>
    <li><a href="#upload-file-menggunakan-cloudinary-dan-streamifier">Upload File Menggunakan Cloudinary dan Streamifier</a></li>
   </ul>
  </details>
...
- <details open>
     ...
     <ul>
       <li>
        <details open>
        <summary><a href="#fitur-login-dan-register-tanpa-global-state">Fitur  Login dan Register Tanpa Global State</a></summary>
         <ul>
...
Buffer adalah area penyimpanan sementara di memori yang digunakan untuk menyimpan data mentah dalam bentuk byte. Dalam Node.js, buffer sering digunakan untuk menangani data biner yang berasal dari file, jaringan, atau sumber lainnya. Berikut fitur utamanya:

- Menyimpan Data Biner: Buffer dapat menyimpan data biner dalam bentuk byte, yang berguna untuk menangani data dari file atau jaringan.
- Operasi Efisien: Buffer memungkinkan operasi efisien pada data biner tanpa perlu mengonversinya ke format lain.
- Interaksi dengan Stream: Buffer sering digunakan bersama stream untuk menyimpan sementara data yang sedang dibaca atau ditulis.

### Stream

Stream adalah abstraksi untuk bekerja dengan data yang dapat dibaca atau ditulis secara berurutan (chunk-by-chunk). Stream memungkinkan Anda untuk memproses data yang sangat besar secara efisien tanpa harus memuat seluruh data ke dalam memori sekaligus. Berikut jenis-jenis stream:

- Readable Stream: Stream yang digunakan untuk membaca data. Contohnya, membaca file atau data dari jaringan.
- Writable Stream: Stream yang digunakan untuk menulis data. Contohnya, menulis data ke file atau mengirim data ke jaringan.
- Duplex Stream: Stream yang dapat dibaca dan ditulis. Contohnya, soket jaringan.
- Transform Stream: Stream yang dapat mengubah data saat sedang dibaca atau ditulis. Contohnya, zlib untuk kompresi.

Berikut fitur utama stream:

- Pemrosesan Data Secara Bertahap: Stream memungkinkan pemrosesan data dalam potongan-potongan kecil (chunk) daripada memproses seluruh data sekaligus.
- Penggunaan Memori yang Efisien: Dengan memproses data dalam chunk, stream mengurangi penggunaan memori dan memungkinkan pemrosesan data yang sangat besar.
- Interaksi dengan Buffer: Stream sering digunakan bersama buffer untuk membaca atau menulis data dalam bentuk byte.
...
1. Buat User Model <br>
   ...
   <a id="tambahkan-variabel-jwt"/>

2. Tambahkan variabel `JWT_SECRET` dan `NODE_ENV` di file `.env`
   ...
   ```
    ...
    <a id="buat-register-controller"/>

3. Buat Controller untuk Registrasi User <br/>
...
4. Buat Router untuk Registrasi <br/>
   ...
   <a id="assign-auth-router"/>

5. Tambahkan middleware cookie parser dan assign auth router di file entry point kita
...
### Register Tanpa Menggunakan Cookies

Untuk membuat fitur register tanpa menggunakan cookies, langkah-langkahnya hampir sama dengan membuat fitur [register menggunakan cookies](#register-menggunakan-cookies). Kita hanya perlu melakukan modifikasi bagian controller mengubah cara pengiriman token JWT ke klien, alih-alih menyimpan token di cookies kita bisa mengirim token langsung di respon JSON. Berikut langkah -langkahnya:

1. Buat User Model, sama seperti di fitur [register dengan cookies](#buat-user-model)
2. Tambahkan variabel `JWT_SECRET` dan `NODE_ENV` di file `.env` sama seperti di fitur [register dengan cookies](#tambahkan-variabel-jwt)
3. Modifikasi Controller [register dengan cookies](#buat-register-controller) jadi seperti ini: <br/>
   Modifikasi Fungsi createSendResToken logika penyimpanan token di cookie dan kirim token secara langsung sebagai bagian dari respons JSON.
...
4. Buat Router untuk Register, sama seperti di fitur [register dengan cookies](#buat-register-router)
5. Assign auth router di file entry point kita, (sudah dilakukan di fitur [register ](#assign-auth-router))

## Fitur Login Menggunakan JWT

### Fitur Login Menggunakan Cookies
...
2. Buat controller Untuk Login <br/>
...
### Fitur Login Tanpa Cookies

1. Tambahkan Method Compare Password Model User, sama seperti di fitur [login menggunakan cookie](#compare-password) <br/>
   Tambahkan method comparePassword untuk membandingkan password yang di requst user (melalui body req) dengan password dari data user yang tersimpan di database.
2. Buat controller Untuk Login <br/>
   Kontroler ini akan mengurus proses login yaitu validation dan juga akan membuat token JWT serta mengirimkannya dalam cookie.
...
### Logout Untuk Login dengan Cookies

Ketika membuat fitur logout untuk yang [loginnya menggunakan cookie](#fitur-login-menggunakan-cookies), Kita (backend) harus membuat API untuk menghapus cookie yang berisi [JWT](https://www.npmjs.com/package/jsonwebtoken) token dengan mengirimkan cookie kosong yang segera kedaluwarsa. Ini dilakukan dengan menetapkan expires pada waktu yang sudah berlalu, sehingga nantinya browser (client / frontend) secara otomatis menghapus cookie token tersebut [ref](https://www.youtube.com/watch?v=fEoI93Z3Efo&list=PLBAY64k6bSAc5xoYSDyh09_1BdfHxwdQY&index=19).

- Buat controller untuk menghapus token di dalam cookie
  ...
  - Fungsi ini menghapus token JWT yang disimpan dalam cookie dengan mengatur cookie jwt menjadi string kosong `('')` dan mengatur tanggal kedaluwarsa cookie (expires) ke waktu saat ini (`Date.now()`).
  - Dengan menghapus atau mengosongkan cookie ini, server memastikan bahwa token JWT yang digunakan untuk autentikasi tidak lagi valid atau dapat digunakan untuk request API lainnya.

- Buat Route untuk menjalankan controller hapus token di cookie tadi
...
### Logout untuk login dan register tanpa cookie

Fitur Logout untuk [login tanpa cookie](#fitur-login-menggunakan-jwt) dan [register tanpa cookie](#register-tanpa-menggunakan-cookies)
kita tidak perlu menghapus token dari cookie karena token tersebut akan disimpan di client-side (misalnya, di localStorage atau sessionStorage). Sehingga kita (backend) tidak perlu membuat api untuk logout ini, karena tokennya tersimpan di sisi client (browser). Itu jadi tugas frontend untuk menghapus token tersebut.

## Middleware Auth Protected

Midleware auth protected merupakan middleware yang akan memeriksa apakah token yang diberikan dalam header Authorization valid dan sesuai. Jika valid, middleware akan melanjutkan request ke endpoint yang direquest. Jika tidak, middleware akan menolak akses dengan mengembalikan pesan error unauthorized yang artinya user belum login.

### Middleware Auth Protected Dengan Cookie

Untuk melindungi endpoint dengan middleware pada aplikasi kita yang menggunakan [JWT](https://www.npmjs.com/package/jsonwebtoken) yang disimpan di dalam cookie. Jadi middleware ini akan mengambil token yang tersimpan dengan di dalam cookie. Dengan token itu kita akan menentukan user tersebut diizinkan untuk mengakses endpoint tersebut atau tidak [ref](https://www.youtube.com/watch?v=fEoI93Z3Efo&list=PLBAY64k6bSAc5xoYSDyh09_1BdfHxwdQY&index=19).

- Buat middleware auth protected
  ...
  - Token Extraction
    - Middleware ini mengecek apakah ada token JWT yang disimpan di cookie jwt.
    - Jika token ada, proses akan berlanjut untuk memverifikasi token tersebut.
  - Token Verification
    - jwt.verify digunakan untuk memverifikasi token menggunakan JWT_SECRET yang disimpan di process.env.JWT_SECRET.
    - Jika token valid, middleware akan mendekode token dan mengambil ID user (decoded.id) dari token.
    - Middleware kemudian mencari user di database berdasarkan ID yang didekode tadi menggunakan User.findById.
    - User ditemukan dan disimpan dalam req.user, tetapi dengan menghapus field password (.select('-password')). Jadi, req.user akan berisi semua data user kecuali kata sandinya

- Tambahkan middleware Auth Protected yang kita buat ke route endpoint yang ingin kita protect.
...
### Middleware Auth Protected Tanpa Cookie

Untuk melindungi endpoint dengan middleware pada aplikasikita yang menggunakan JWT (JSON Web Token) tanpa cookie, Anda perlu membuat middleware yang akan memeriksa apakah token yang diberikan dalam header Authorization valid dan sesuai. Jika valid, middleware akan melanjutkan permintaan ke endpoint yang direquest. Jika tidak, middleware akan menolak akses dengan mengembalikan pesan error unauthoirzed yang berarti user tersebut belum login.

- Buat middleware auth protected<br/>
  Middleware ini akan memeriksa token yang ada di header Authorization dan memverifikasinya.

  ```js
  // src/middlewares/authWithoutCookieMiddleware.js
...
- Tambahkan middleware Auth Protected yang kita buat ke route endpoint yang ingin kita protect.
  ...
  router.get(
    '/without-cookie',
    protectedWithoutCookieMiddleware,
    GetAllProduct
  );
  //-----------------------------------------------------------------------------------------------
  ...
  - `req.headers.authorization`: Mengecek apakah header Authorization ada pada request.
  - `req.headers.authorization.startsWith('Bearer')`: Mengecek apakah token dimulai dengan kata "Bearer", yang merupakan konvensi untuk menunjukkan bahwa ini adalah token bearer.
  - `token = req.headers.authorization.split(' ')[1]`: Ekstraksi token dari header dengan mengabaikan kata "Bearer" dan hanya mengambil nilai tokennya.
  - `jwt.verify(token, process.env.JWT_SECRET)`: Verifikasi token menggunakan JWT_SECRET yang disimpan dalam variabel lingkungan (.env). JWT_SECRET adalah kunci rahasia yang digunakan untuk memverifikasi keaslian token. Hasil verifikasi disimpan dalam variabel decoded.
  - `User.findById(decoded.id)`: Mengambil data pengguna dari database berdasarkan ID yang terdapat di dalam token (decoded.id).
  - `select('-password')`: Menghilangkan password dari objek pengguna yang dikembalikan, sehingga password tidak ikut terkirim ke frontend.
  - `req.user`: Menyimpan data pengguna yang ditemukan ke dalam req.user, sehingga data ini dapat digunakan di middleware atau controller berikutnya.
...
## Get All Orders, Get Order By Id dan Get Current User Order

Berikut adalah langkah-langkah untuk membuat endpoint yang memungkinkan user untuk mengambil semua order, order berdasarkan ID, dan order user saat ini. Proses ini mencakup modifikasi controller, pembuatan router, dan integrasi router tersebut ke dalam file entry point aplikasi [ref](https://www.youtube.com/watch?v=-xqNKWoPAxw&list=PLBAY64k6bSAc5xoYSDyh09_1BdfHxwdQY&index=9).

1. Modifikasi Controller Order<br/>
   Pertama, kita modifikasi controller untuk menangani request mendapatkan semua order, order berdasarkan ID, dan order user saat ini.
...
2. Modifikasi Router Order<br/>
   Selanjutnya, kita buat router untuk menghubungkan endpoint ke controller yang telah dimodifikasi.
...
## Gunakan Middleware Express Mongo Sanitize Dan Helmet

Berikut kita akan bahas keduanya [ref](https://www.youtube.com/watch?v=F7JcskNKHQs&list=PLBAY64k6bSAc5xoYSDyh09_1BdfHxwdQY&index=8).

### Express Mongo Sanitize

[express-mongo-sanitize](https://www.npmjs.com/package/express-mongo-sanitize) adalah sebuah middleware untuk Express yang membantu melindungi aplikasi dari serangan NoSQL injection. Package ini bekerja dengan cara membersihkan data yang masuk dari request body, query string, dan parameters yang dapat digunakan untuk melakukan injeksi ke database MongoDB.

Kegunaan middleware express mongo sanitize ini adalah:

- Keamanan<br/>
  Melindungi aplikasi dari serangan NoSQL injection yang dapat mengekspos atau merusak data.
- Integrasi Mudah<br/>
  Mudah diintegrasikan dengan aplikasi Express.
- Pembersihan Data<br/>
  Membersihkan data yang berpotensi berbahaya sebelum mencapai database.

### Helmet

[helmet](https://www.npmjs.com/package/helmet) adalah sebuah package yang membantu meningkatkan keamanan aplikasi Express dengan mengatur berbagai header HTTP yang relevan. Header-header ini dapat membantu melindungi aplikasi dari beberapa kerentanan web umum.

Berikut manfaat middleware helmet:

- Keamanan Tambahan<br/>
  Menambahkan lapisan keamanan tambahan dengan mengatur header HTTP yang aman.
- Best Practices<br/>
  Mengikuti praktik terbaik dalam keamanan aplikasi web.
- Konfigurasi Mudah<br/>
  Mudah untuk dikonfigurasi dan diterapkan pada aplikasi Express.

### Penggunaan Express Mongo Sanitize dan Helmet

Setelah menginstall kedua package tersebut, import dan inisialisasi di file entry point kita.
...
## Upload File dengan Cloudinary dan Streamifier di ExpressJS

Kita akan melakukan uplod file menggunakan [cloudinary](https://cloudinary.com/documentation/react_integration) dan [streamifier](https://www.npmjs.com/package/streamifier).

### Cloudinary

Cloudinary adalah layanan penyimpanan berbasis cloud yang dirancang khusus untuk menangani dan mengelola media, seperti gambar dan video. Dengan Cloudinary, Anda dapat mengunggah, menyimpan, mengelola, dan mengirimkan media secara efisien. Berikut fitur Utamanya:

- Penyimpanan Media: Menyimpan gambar, video, dan file lainnya di cloud.
- Transformasi Gambar: Mengubah ukuran, memotong, mengubah format, dan menerapkan efek pada gambar secara dinamis.
- Pengiriman Cepat: Menggunakan CDN untuk mengirimkan media dengan cepat ke pengguna di seluruh dunia.
- Pengelolaan Media: Mengatur dan mengelola koleksi media melalui dashboard atau API.

### Streamifier

Streamifier adalah package Node.js yang memungkinkan Anda mengubah buffer atau string menjadi stream. Stream adalah konsep di Node.js untuk menangani data yang dapat diambil secara berurutan, memungkinkan pemrosesan data yang efisien dan tidak perlu menunggu seluruh data untuk diterima sebelum mulai memprosesnya. Berikut fitur utamanya:

- Konversi Buffer ke Stream: Mengubah buffer menjadi stream yang dapat dibaca.
- Konversi String ke Stream: Mengubah string menjadi stream yang dapat dibaca.

### Upload File Menggunakan Cloudinary dan Streamifier

Berikut adalah langkah-langkah untuk mengimplementasikan upload file ke Cloudinary menggunakan ExpressJS:

1. Buat Utilitas Menggunakan multer <br/>
   Pertama, kita perlu membuat utilitas untuk menangani upload file di server. Utilitas ini akan menggunakan multer, sebuah middleware untuk menangani multipart/form-data, yang terutama digunakan untuk mengunggah file.

   ```js
   // utils/cloudinaryUploadFileHandler.js
   ...
   Dalam konfigurasi ini, `multer.memoryStorage()` digunakan untuk menyimpan file di memori. Ini berguna ketika kita ingin memproses file langsung tanpa menyimpannya di disk.

2. Buat Controller untuk Logika Upload File ke Cloudinary <br/>
   Berikutnya, kita perlu membuat controller yang akan menangani logika upload file ke Cloudinary. Controller ini akan menggunakan Cloudinary SDK untuk mengupload file yang diterima dari multer.
   ...
   export const CloudinaryUploadImage = asyncHandler(async (req, res) => {
     //membuat stream untuk mengupload file ke Cloudinary
     const stream = cloudinary.uploader.upload_stream(
       {
         // Folder di clodinary yang akan dijadikan tempat menyimpan file
         folder: 'uploads',
         // Format file yang diperbolehkan untuk diupload
         allowed_formats: ['jpg', 'svg', 'png'],
         // Untuk menentukan nama file yang akan tersimpan di cloudinary
     ...
     // Mengubah buffer file menjadi stream dan mengunggahnya/s
     streamifier.createReadStream(req?.file?.buffer).pipe(stream);
   });
   //-----------------------------------------------------------------------------------------------------------------
   ```

3. Buat Router untuk Upload File ke Cloudinary <br/>
   Terakhir, kita perlu membuat router yang akan mengatur endpoint untuk upload file ke Cloudinary. Router ini akan menggunakan middleware `multer` yang telah dikonfigurasi sebelumnya, serta controller yang telah kita buat tadi.
...
Jika kita mengalami masalah CORS (Cross-Origin Resource Sharing) di backend, yaitu ketika frontend mengakses API yang kita buat. Kita perlu mengatur server backend agar mengizinkan permintaan dari domain lain. Ini bisa dilakukan dengan menambahkan middleware CORS di backend kita.

Langkah-langkah untuk Mengatasi Masalah CORS di Backend:

1. Install CORS Middleware<br/>
   install middleware cors dengan command
   ```ts
   npm install cors
   ```
2. Gunakan CORS Middleware di Express<br/>
   Tambahkan middleware cors di file utama backend Anda (biasanya `app.js`, `index.js` atau `server.js`).
